...
g.OPERAND = function(e, r) {
  ...
  if (Array.isArray(r))
    ...
    t = t.concat(g.NUMBER(e));
  else if (r === "offset")
    t = t.concat(g.NUMBER32(e));
  else if (r === "number")
    t = t.concat(g.NUMBER(e));
  else if (r === "real")
    t = t.concat(g.REAL(e));
...
function z(e, r, t) {
  ...
  if (this.tableName = e, this.fields = r, t)
    for (var s = Object.keys(t), i = 0; i < s.length; i += 1) {
      var u = s[i], o = t[u];
...
function Re(e, r, t) {
  ...
  for (var n = 0; n < r.length; n++)
    a[n + 1] = { name: e + n, type: "USHORT", value: r[n] };
  return a;
}
function dr(e, r, t) {
  ...
  for (var s = 0; s < a; s++)
    n[s + 1] = { name: e + s, type: "TABLE", value: t(r[s], s) };
  return n;
}
function we(e, r, t) {
  var a = r.length, n = [];
...
v.prototype.parseCoverage = function() {
  ...
  if (r === 1)
    return { format: 1, glyphs: this.parseUShortList(t) };
  if (r === 2) {
    for (var a = new Array(t), n = 0; n < t; n++)
      a[n] = { start: this.parseUShort(), end: this.parseUShort(), index: this.parseUShort() };
...
v.prototype.parseClassDef = function() {
  ...
  if (r === 2)
    return { format: 2, ranges: this.parseRecordList({ start: v.uShort, end: v.uShort, classId: v.uShort }) };
...
Q.prototype.getPath = function(e, r, t, a, n) {
  ...
  if (a.hinting && n && n.hinting && (i = this.path && n.hinting.exec(this, t)), i)
    s = n.hinting.getCommands(i), e = Math.round(e), r = Math.round(r), u = o = 1;
  else {
...
Q.prototype.drawPoints = function(e, r, t, a) {
  function n(h, p, c, d) {
    ...
    for (var x = 0; x < h.length; x += 1)
      e.moveTo(p + h[x].x * d, c + h[x].y * d), e.arc(p + h[x].x * d, c + h[x].y * d, 2, 0, Math.PI * 2, false);
    e.closePath(), e.fill();
  }
...
Cr.prototype.get = function(e) {
  if (this.glyphs[e] === void 0) {
    ...
    if (t)
      for (var a = 0; a < t.unicodes.length; a++)
...
function ja(e, r) {
  var t, a, n, s;
  if (r === 28)
    return t = e.parseByte(), a = e.parseByte(), t << 8 | a;
  if (r === 29)
    return t = e.parseByte(), a = e.parseByte(), n = e.parseByte(), s = e.parseByte(), t << 24 | a << 16 | n << 8 | s;
  if (r === 30)
    return Ja(e);
  if (r >= 32 && r <= 246)
    return r - 139;
...
function $a(e) {
  for (var r = {}, t = 0; t < e.length; t += 1) {
    ...
    if (n.length === 1 ? s = n[0] : s = n, r.hasOwnProperty(a) && !isNaN(r[a]))
      throw new Error("Object " + r + " already has key " + a);
    r[a] = s;
  }
  return r;
}
...
function Dt(e, r, t) {
  for (var a = {}, n, s = 0; s < r.length; s += 1) {
    ...
    if (Array.isArray(i.type)) {
      ...
      n = e[i.op], n === void 0 && (n = i.value !== void 0 ? i.value : null), i.type === "SID" && (n = Oe(t, n)), a[i.name] = n;
  }
  return a;
}
function en(e, r) {
...
function _r(e, r, t) {
  ...
  if (e.isCIDFont) {
    ...
    d = e.tables.cff.topDict._subrs, x = e.tables.cff.topDict._subrsBias, m = e.tables.cff.topDict._defaultWidthX, y = e.tables.cff.topDict._nominalWidthX;
  var R = m;
  function O(F, G) {
...
function nn(e, r, t, a) {
  ...
  if (u === 0)
    for (var o = 0; o < t; o++) {
      if (s = i.parseCard8(), s >= a)
        throw new Error("CFF table CID Font FDSelect has bad FD index value " + s + " (FD count " + a + ")");
      n.push(s);
    }
  else if (u === 3) {
    ...
    for (var h, p = 0; p < l; p++) {
      if (s = i.parseCard8(), h = i.parseCard16(), s >= a)
        throw new Error("CFF table CID Font FDSelect has bad FD index value " + s + " (FD count " + a + ")");
      if (h > t)
...
function sn(e, r, t, a) {
  ...
  if (f.encoding === 0 ? t.cffEncoding = new je(Na, R) : f.encoding === 1 ? t.cffEncoding = new je(Ha, R) : t.cffEncoding = an(e, r + f.encoding, R), t.encoding = t.encoding || t.cffEncoding, t.glyphs = new ue.GlyphSet(t), a.lowMemory)
    t._push = function(L) {
...
function hn(e) {
  ...
  for (var a = 0, n = 0, s = 0; s < t.commands.length; s += 1) {
    ...
    if (o.type === "M")
      i = Math.round(o.x - a), u = Math.round(o.y - n), r.push({ name: "dx", type: "NUMBER", value: i }), r.push({ name: "dy", type: "NUMBER", value: u }), r.push({ name: "rmoveto", type: "OP", value: 21 }), a = Math.round(o.x), n = Math.round(o.y);
    else if (o.type === "L")
      i = Math.round(o.x - a), u = Math.round(o.y - n), r.push({ name: "dx", type: "NUMBER", value: i }), r.push({ name: "dy", type: "NUMBER", value: u }), r.push({ name: "rlineto", type: "OP", value: 5 }), a = Math.round(o.x), n = Math.round(o.y);
    else if (o.type === "C") {
...
function Sn(e, r, t, a, n) {
  ...
  for (var s, i, u = new k.Parser(r, t), o = 0; o < n; o += 1)
    o < a && (s = u.parseUShort(), i = u.parseShort()), e._hmtxTableData[o] = { advanceWidth: s, leftSideBearing: i };
}
...
function Bn(e, r) {
  ...
  for (var G = new b.Table("name", [{ name: "format", type: "USHORT", value: 0 }, { name: "count", type: "USHORT", value: f.length }, { name: "stringOffset", type: "USHORT", value: 6 + f.length * 12 }]), Y = 0; Y < f.length; Y++)
    G.fields.push({ name: "record_" + Y, type: "RECORD", value: f[Y] });
...
ee[1] = function() {
  ...
  if (t === 1)
    return { substFormat: 1, coverage: this.parsePointer(v.coverage), deltaGlyphId: this.parseUShort() };
  if (t === 2)
    return { substFormat: 2, coverage: this.parsePointer(v.coverage), substitute: this.parseOffset16List() };
...
ee[5] = function() {
  var r = this.offset + this.relativeOffset, t = this.parseUShort();
  if (t === 1)
    return { substFormat: t, coverage: this.parsePointer(v.coverage), ruleSets: this.parseListOfLists(function() {
      var s = this.parseUShort(), i = this.parseUShort();
      return { input: this.parseUShortList(s - 1), lookupRecords: this.parseRecordList(i, ke) };
    }) };
  if (t === 2)
    return { substFormat: t, coverage: this.parsePointer(v.coverage), classDef: this.parsePointer(v.classDef), classSets: this.parseListOfLists(function() {
      var s = this.parseUShort(), i = this.parseUShort();
...
ee[6] = function() {
  var r = this.offset + this.relativeOffset, t = this.parseUShort();
  if (t === 1)
    return { substFormat: 1, coverage: this.parsePointer(v.coverage), chainRuleSets: this.parseListOfLists(function() {
  ...
  if (t === 2)
    return { substFormat: 2, coverage: this.parsePointer(v.coverage), backtrackClassDef: this.parsePointer(v.classDef), inputClassDef: this.parsePointer(v.classDef), lookaheadClassDef: this.parsePointer(v.classDef), chainClassSet: this.parseListOfLists(function() {
      return { backtrack: this.parseUShortList(), input: this.parseUShortList(this.parseShort() - 1), lookahead: this.parseUShortList(), lookupRecords: this.parseRecordList(ke) };
    }) };
  if (t === 3)
    return { substFormat: 3, backtrackCoverage: this.parseList(v.pointer(v.coverage)), inputCoverage: this.parseList(v.pointer(v.coverage)), lookaheadCoverage: this.parseList(v.pointer(v.coverage)), lookupRecords: this.parseRecordList(ke) };
...
Ue[6] = function(r) {
  if (r.substFormat === 1) {
    ...
    U.assert(false, "lookup type 6 format 2 is not yet supported.");
...
function jt(e) {
  ...
  for (var a = [], n = [], s = r.sizeOf() + Qr().sizeOf() * r.numTables; s % 4 !== 0; )
    s += 1, n.push({ name: "padding", type: "BYTE", value: 0 });
  for (var i = 0; i < e.length; i += 1) {
    var u = e[i];
    U.argument(u.tableName.length === 4, "Table name" + u.tableName + " is invalid.");
    var o = u.sizeOf(), l = Qr(u.tableName, Er(u.encode()), s, o);
    for (a.push({ name: l.tag + " Table Record", type: "RECORD", value: l }), n.push({ name: u.tableName + " table", type: "RECORD", value: u }), s += o, U.argument(!isNaN(s), "Something went wrong calculating the offset."); s % 4 !== 0; )
      s += 1, n.push({ name: "padding", type: "BYTE", value: 0 });
  }
...
function qn(e) {
  for (var r = [], t = [], a = [], n = [], s = [], i = [], u = [], o, l = 0, f = 0, h = 0, p = 0, c = 0, d = 0; d < e.glyphs.length; d += 1) {
    ...
    if (isNaN(x.advanceWidth))
      throw new Error("Glyph " + x.name + " (" + d + "): advanceWidth is not a number.");
  ...
  for (var rr = jt(q), ha = rr.encode(), ca = Er(ha), tr = rr.fields, Ar = false, Ge = 0; Ge < tr.length; Ge += 1)
...
Ae.prototype = { searchTag: sr, binSearch: Jr, getTable: function(e) {
  ...
  if (a) {
    if (!r || r === "dflt" || r === "DFLT")
      return a.defaultLangSys;
    var n = sr(a.langSysRecords, r);
    if (n >= 0)
      return a.langSysRecords[n].langSys;
    if (t) {
...
K.prototype.addLigature = function(e, r, t, a) {
  ...
  if (l >= 0) {
    for (var f = s.ligatureSets[l], h = 0; h < f.length; h++)
      if (Yn(f[h].components, u))
        return;
    f.push(o);
  } else
    l = -1 - l, s.coverage.glyphs.splice(l, 0, i), s.ligatureSets.splice(l, 0, [o]);
};
...
function ea(e, r, t) {
  ...
  if (e.numberOfContours > 0) {
    ...
    n = [];
    for (var f = 0; f < l; f += 1)
      if (s = a.parseByte(), n.push(s), (s & 8) > 0)
        for (var h = a.parseByte(), p = 0; p < h; p += 1)
          n.push(s), f += 1;
...
function ra(e) {
  ...
  for (var t = Kn(e), a = 0; a < t.length; ++a) {
    ...
    for (var l = 0; l < n.length; ++l)
      if (s = i, i = u, u = n[(l + 1) % n.length], i.onCurve)
        r.lineTo(i.x, i.y);
      else {
        var f = s, h = u;
  ...
  if (r.isComposite)
    for (var t = 0; t < r.components.length; t += 1) {
      ...
      if (n.getPath(), n.points) {
        var s = void 0;
        if (a.matchedPoints === void 0)
          s = ir(n.points, a);
        else {
          if (a.matchedPoints[0] > r.points.length - 1 || a.matchedPoints[1] > n.points.length - 1)
...
ia.prototype.exec = function(e, r) {
  ...
  if (!(this._errorState > 2)) {
    ...
    if (!a || a.ppem !== r) {
      ...
      var s = t.tables.cvt;
      if (s)
        for (var i = a.cvt = new Array(s.length), u = r / t.unitsPerEm, o = 0; o < s.length; o++)
          i[o] = s[o] * u;
      else
        a.cvt = [];
...
sa = function(e, r) {
  ...
  if (de.prototype = r, !n)
    u = new de("glyf", e.instructions), exports.DEBUG && (console.log("---EXEC GLYPH---"), u.step = -1), br(e, u, t, a), i = u.gZone;
  else {
    var o = r.font;
    i = [], s = [];
    for (var l = 0; l < n.length; l++) {
...
Se = function(e) {
  ...
  if (!!r) {
    ...
    for (e.ip = 0; e.ip < t; e.ip++) {
      if (exports.DEBUG && e.step++, a = na[r[e.ip]], !a)
        throw new Error("unknown instruction: 0x" + Number(r[e.ip]).toString(16));
      a(e);
    }
  }
};
...
function it(e, r) {
  ...
  for (var u = 0; u < a; u++)
    n = t[u], !e.touched(n) && (s = n.prevTouched(e), s !== n && (i = n.nextTouched(e), s === i && e.setRelative(n, n, e.distance(s, s, false, true), e, true), e.interpolate(n, s, i, e)));
}
function ot(e, r) {
...
function lt(e, r) {
  ...
  for (var l, f = i.distance(n, n, false, true), h = o.length - 2, p = 0; p < h; p++)
...
B.prototype.composeRUD = function(e) {
  ...
  if (a.every(n))
    return { FAIL: "composeRUD: one or more operations hasn't completed successfully", report: a.filter(n) };
...
function Xi(e, r, t) {
  ...
  for (var n = e.substitution.components.length, s = 0; s < n; s++)
    a = r[t + s + 1], a.setState("deleted", true);
}
...
var vo = function() {
  ...
  if (U.argument(e === 1 || e === 2 || e === 3, "Unsupported CaretValue table version."), e === 1)
...
te[1] = function() {
  var r = this.offset + this.relativeOffset, t = this.parseUShort();
  if (t === 1)
    return { posFormat: 1, coverage: this.parsePointer(v.coverage), value: this.parseValueRecord() };
  if (t === 2)
    return { posFormat: 2, coverage: this.parsePointer(v.coverage), values: this.parseValueRecordList() };
...
te[2] = function() {
  ...
  if (t === 1)
    return { posFormat: t, coverage: a, valueFormat1: n, valueFormat2: s, pairSets: this.parseList(v.pointer(v.list(function() {
...
function I(e, r) {
  if (r.compression === "WOFF") {
    ...
    if (Ta(t, a), a.byteLength !== r.length)
...
function Dr(e, r) {
  ...
  if (o === String.fromCharCode(0, 1, 0, 0) || o === "true" || o === "typ1")
    n.outlinesFormat = "truetype", i = k.getUShort(s, 4), u = yt(s, i);
  else if (o === "OTTO")
    n.outlinesFormat = "cff", i = k.getUShort(s, 4), u = yt(s, i);
  else if (o === "wOFF") {
    var l = k.getTag(s, 4);
...
//# sourceMappingURL=opentype-B5LDPJTC.js.map
